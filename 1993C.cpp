...
void coutData(const Container &container) {
    ...
    int main() {
        ...
        while (t--) {
            ...
            for(int i=0; i<n; i++){
               vp[i].first = (maxi-v[i])/k;
               vp[i].second = (maxi-v[i])%k; 
            }
           //coutData(vp);
        //    for(int i=0; i<n; i++){
        ...
        //    }
           ...
           for(int i=0; i<n; i++){
                if(vp[i].first %2 == 1){
                    if(k-vp[i].second > maxiforeven){
                        maxiforeven = k-vp[i].second;
                    }
                }
           }
           ll maxiposs = INT_MAX;
           for(int i=0; i<n; i++){
                if(vp[i].first %2 == 0){
                    if(k-vp[i].second < maxiposs){
                        maxiposs = k-vp[i].second;
                    }
                }
           }       
        //cout<<maxiforeven<<" "<<maxiposs;
           if(maxiforeven<maxiposs){
            cout<<maxi+maxiforeven<<endl;
           }
           else{
            cout<<-1<<endl;
           }
