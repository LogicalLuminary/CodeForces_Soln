...
bool isallEven(vector<ll> &v,ll n){
    bool all = true;
    for(int i=0; i<n; i++){
...
bool isallOdd(vector<ll> &v,ll n){
    bool all = true;
    for(int i=0; i<n; i++){
        if(v[i]%2 == 0) return false;
    }
...
bool alleq(vector<ll> &v, ll n){
    ...
    int main() {
        ...
        if(isallEven(v,n) || isallOdd(v,n)){
            int done=0;
            while(! alleq(v,n) && done<39){

            ll maxidx = max_element(all(v))-v.begin();
            ll max = v[maxidx];
            for( int i=0; i< n; i++){
                v[i] = abs(v[i]-max/2);
                //cout<<maxidx<<" ";
        ...
        // else if(isallOdd(v,n)){
            
        //     while(! alleq(v,n)){
        //      ll maxidx = max_element(all(v))-v.begin();
        //     for(int i=0; i<n&&i<39; i++){
        //         v[i] = abs(v[i]-(v[maxidx])/2);
        //         //cout<<maxidx<<" ";
        //     }
        //     v2.pb(v.at(maxidx));
        //     //coutData(v);
