...
void coutData(const Container &container) {
    ...
    int main() {
        ...
        while (t--) {
            ...
            ll n;
            vector<pair<ll,ll>> vp1(n);
            for(int i=0; i<n; i++){
                cin>>vp1[i].first;
            }
            vector<pair<ll,ll>> vp2(n);
            for(int i=0; i<n; i++){
                vp2[i].first = vp1[i].first;
                vp2[i].second = i;
            }
        ...
        int i=0,j=0;
        vector<pair<ll,ll>> toinc;

        for( i; i<n;i++){

            if(vp2[i].first == vp1[i].first){i++;j++;}

            else if(vp2[i].first < vp1[i].first){
                toinc.pb({j,vp2[i].second});
                i++;
            }
