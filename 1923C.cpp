...
int main() {
    ...
    while (t--) {
           ...
           vec sumarr(n);
           sumarr[0]= v[0];
           for(int i=1;i<n;i++) sumarr[i]=sumarr[i-1]+v[i];
           vec ones(n);
           ll c=0;
           for(int i=0; i<n;i++){
            if(v[i] == 1)c++;
           ...
           for( auto e : vp){
                int i = e.first-1;
                int j = e.second-1; // coz indices

                // vec t(v.begin()+i,v.begin()+j+1);
                // coutData(t);
                ll len = j-i+1;
                if(len ==1) {cout<<"NO\n";continue;}
                // ll sum = accumulate(all(t),0LL);
                ll sum = sumarr[j]-sumarr[i]+v[i];
                // cout<<sum<<endl;
                if(2*sum <3*len ) cout<<"NO\n";
                else if(sum<2*len){
                    ll oneC = ones[j]-ones[i];
                    if(v[i] ==1) oneC ++;
                    ll profit = sum - oneC - (len-oneC);
                    if(profit >= oneC) cout<<"YES\n";
                    else cout<<"NO\n";
                }
                else cout<<"YES\n";
           }
