...
int main() {
    ...
    while (t--) {
        ...
        //coutData(va);
            ...
            ll maxpairidx =-1;
            for(int i=0; i<n-1; i++){
                if(va[i] == va[i+1]) {maxpair = va[i] ; maxpairidx = i ;};
            }
        
            //cout<<maxpair<<" "<<maxpairidx<<endl;
            ll mindif = INT_MAX;
            ll idx;
    int i = maxpairidx; 
    if (i >= 0 && i + 1 < va.size()) {
...
n=va.size();
        for(int i=0; i<n-1; i++){
            if(va[i+1]-va[i] < mindif) {mindif = va[i+1]-va[i];idx =i;}
        }
//cout<<mindif<<" "<<idx<<endl;;
    if(maxpair != -1){

        if(va[idx+1]-va[idx] < 2*maxpair){
            cout<< maxpair<<" "<<maxpair<<" "<< va[idx+1]<<" "<<va[idx]<<endl;
        }
        else{
            cout<<-1<<endl;
        }
